...
/**
 * Manual, real-hardware verification that the local drawing primitives (doc/PROTOCOL.md
 * §12.2-§12.5) work over the BLE transport specifically - the BLE counterpart of
 * {@link DrawPrimitivesManualCheck}, which only ever exercised Serial. Not part of the automated
 * {@code mvn test} suite (real device required). Scans for a device advertising the CrowPanel
 * service UUID, connects via {@link BleHmiDevice}, then sends the exact same deferred
 * rect/rect(XOR)/circle/circle/line sequence followed by one {@code REFRESH} as
 * {@link DrawPrimitivesManualCheck} does, so a passing run is directly comparable to the
 * already-verified Serial result:
 *
 * <pre>
 ...
 *
 * <p>
 * With no argument, connects to the first device found advertising the CrowPanel service UUID -
 * fine with exactly one such device in range. With multiple CrowPanel units powered on at once,
 * pass a substring of the target's address (e.g. {@code A5:A3:65}) or advertised name (e.g.
 * {@code A5A364}) to pick a specific one instead of racing the scan order.
 *
 * <p>
 * Uses the <em>same</em> {@link BleAdapter} instance for both the scan and the
 * {@link BleHmiDevice} it hands the discovered address to - see {@link
 * cz.bliksoft.hmieink.protocol.BleFrameTransport}'s class doc for why that's required.
 */
public final class DrawPrimitivesBleManualCheck {
	...
	public static void main(String[] args) throws Exception {
		...
		try (BleAdapter adapter = new BleAdapter()) {
			System.out.println("Scanning for service " + Ble.SERVICE_UUID
					+ (target != null ? " matching \"" + target + "\"" : "") + " (up to " + SCAN_TIMEOUT_MS + " ms)...");
			adapter.scan(new ScanFilter().withServiceUuid(Ble.SERVICE_UUID), SCAN_TIMEOUT_MS, (address, name, rssi) -> {
				...
				boolean matches = target == null
						|| address.toUpperCase(java.util.Locale.ROOT).contains(target)
