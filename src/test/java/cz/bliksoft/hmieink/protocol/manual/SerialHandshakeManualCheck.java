...
/**
 * Manual, real-hardware verification for {@link SerialFrameTransport} and
 * {@link CommandClient}: sends HANDSHAKE_REQUEST over a real serial port via
 * the stop-and-wait command client and decodes the response through
 * {@link HandshakeCapabilities}. NOT part of the automated `mvn test` suite
 * (this class's name doesn't match Surefire's default *Test.java / Test*.java
 * patterns, and it requires a real board attached) - run it directly:
 *
 * <pre>
