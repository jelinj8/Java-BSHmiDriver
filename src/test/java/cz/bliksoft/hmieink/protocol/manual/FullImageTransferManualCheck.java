...
/**
 * Manual, real-hardware verification of FULL_IMAGE_TRANSFER (doc/PROTOCOL.md §6, next-steps.md
 * #4): sends a real 400x300 test pattern (horizontal black/white stripes, RLE-encoded) over Serial
 * via {@link CommandClient} and checks the device replies ACK. Does NOT verify pixels actually
 * appeared correctly - that still needs a human looking at the panel, same as
 * {@code displaySelfTest()}'s bring-up check. NOT part of the automated {@code mvn test} suite -
 * run it directly:
 *
 * <pre>
...
public final class FullImageTransferManualCheck {
	...
	// Hardcoded to this board's panel (doc/PROTOCOL.md §6 has no width/height fields - it always
	// targets the full panel at the handshake-reported resolution; a real client would read this
	// from HANDSHAKE_RESPONSE's DISPLAY_WIDTH_PX/HEIGHT_PX TLVs instead of hardcoding it here).
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			try {
				...
				System.out.println(
						"OK: device replied 0x" + Integer.toHexString(response.getCommandId())
	...
	// Wire polarity (doc/PROTOCOL.md §6): bit=1=BLACK, bit=0=WHITE, row-major top-to-bottom,
