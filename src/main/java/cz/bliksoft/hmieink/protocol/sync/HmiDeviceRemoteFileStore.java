...
/**
 * Production {@link RemoteFileStore}: binds a live {@link HmiDevice} to one VOLUME + base
 * device-side path (doc/PROTOCOL.md §14). {@link RemoteFileStore}'s relative paths are joined onto
 * {@code basePath} to form the absolute device path each wire command needs.
 */
public final class HmiDeviceRemoteFileStore implements RemoteFileStore {

	// Real SD hardware genuinely takes longer than CommandClient.DEFAULT_TIMEOUT_MILLIS (5s) for a
	// large directory listing or file transfer - every VOLUME=SD operation walks the FAT directory
	// over SPI (doc/PROTOCOL.md §14) - measured directly against a ~220-entry generated font glyph
	// set (design note 96): a single FILE_LIST_REQUEST for that many entries reliably exceeded the
	// default timeout (even after its one built-in retry), though the top-level, few-entry listing
	// and individual small file transfers stayed well within it. A generous flat ceiling here, not a
	// per-entry-count estimate, since FolderSync has no cheap way to know a remote directory's size
	// before listing it.
	...
	// A not-yet-existing directory (FILE_NOT_FOUND, doc/PROTOCOL.md §14.1) is treated as empty rather
	// than propagated - the natural reading for a first-ever sync into a device folder that doesn't
	// exist yet, since FILE_UPLOAD (§14.3) auto-creates it (and any deeper missing parents) on the
